...
public class AutoLoadingIT {
    ...
    @Test
    @DisplayName("ZooKeeper job lock module must auto-load without startup errors")
    public void autoLoaded() {

        // Not resolving LockHandler from DI on purpose: keeping this test symmetric with the Consul one
        boolean loaded = app.getInstance(ModulesMetadata.class).getModules().stream()
                .map(ModuleMetadata::getName)
                .anyMatch("ZkJobModule"::equals);

        assertTrue(loaded, "ZkJobModule was not auto-loaded");
